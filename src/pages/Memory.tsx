    ...
    >
      ...
      <div className="mb-4">
        <HowItWorksBlock
          ...
          intro="Альбом поколений — третий слой семейного кода. Древо отвечает на вопрос «кто мы», Дорога жизни — «что мы прошли», а Альбом — «что мы чувствовали». Это не очередное облачное хранилище и не лента соцсети: здесь у каждой фотографии есть автор, герои из вашего древа, дата, место и история. Через 30 лет внуки откроют профиль прабабушки — и увидят не пустую ячейку, а живые мгновения."
          steps={[
            { icon: 'Plus', title: 'Шаг 1. Добавьте память', description: 'Нажмите «Добавить память». Загрузите до 10 фото, напишите короткую историю, поставьте дату и место — это превращает снимок в свидетельство эпохи.' },
            { icon: 'GitBranch', title: 'Шаг 2. Свяжите с Древом', description: 'Отметьте, кто на фото. Память автоматически появится в карточке каждого человека в Древе семьи — даже у тех, кого уже нет рядом.' },
            { icon: 'Milestone', title: 'Шаг 3. Привяжите к Дороге жизни', description: 'Выберите событие («Свадьба», «Рождение Маши», «Переезд») — память встанет на свою точку на Дороге жизни и обогатит общую хронику семьи.' },
            { icon: 'FolderHeart', title: 'Шаг 4. Соберите альбом', description: 'Группируйте карточки в тематические альбомы: «Лето на даче», «Школа», «90-е». Один клик — и на стене семьи появляется готовая глава вашей истории.' },
            { icon: 'Search', title: 'Шаг 5. Находите за секунды', description: 'Фильтры по человеку, событию, году и полнотекстовый поиск — найдёте нужный момент через 20 лет, даже если забыли точную дату.' },
            { icon: 'Sparkles', title: 'В чём уникальность?', description: 'Ни одна соцсеть не связывает фото с генеалогией и таймлайном жизни. Здесь память не теряется в ленте — она становится частью родового кода и наследуется детьми.' },
          ]}
