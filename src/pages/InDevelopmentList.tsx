...
const devSections: DevSection[] = [
  ...
  {
    id: 'ai-memory',
    title: 'AI-память семьи (RAG + Библиотека знаний)',
    description: 'Контекстный AI-слой под управлением Домового',
    icon: 'BrainCircuit',
    features: [
      '🔑 Семейный ID как ключ — Под наследуемым ключом семьи Домовой собирает и хранит весь контекст: память, документы, события, цели',
      '📚 Библиотека знаний семьи — Единое хранилище: документы, история, события, переписка, цели, фото, заметки. Растёт со временем',
      '🧠 RAG-движок — Контекстный поиск и извлечение знаний для AI: Домовой отвечает не «вообще», а в контексте именно этой семьи',
      '🔍 Семейный поиск — «Когда мы последний раз ездили к бабушке?», «Что Маша любила в 5 лет?», «Какие традиции были у дедушки?»',
      '💬 Контекстные ответы Домового — Помощник знает реальную историю и события семьи, а не generic ответы LLM',
      '⏰ Умные напоминания — На основе паттернов и истории семьи: годовщины, ритуалы, важные даты',
      '👨‍👩‍👧‍👦 Передача между поколениями — Дети и внуки получают доступ к памяти рода: фото, истории, ценности, традиции',
      '📖 Семейная летопись — Автоматическое формирование истории рода из накопленного контекста',
      '🛡️ Приватность семейных данных — Данные семьи доступны только тем, кому семья открыла Семейный ID',
      '🎯 Персональные инсайты — AI-инсайты по каждому члену семьи на основе долгосрочного контекста',
      '🔄 Накопительная ценность — Чем дольше семья пользуется, тем глубже контекст и выше LTV',
      '🌳 Граф знаний семьи — Связи между людьми, событиями, документами и историями',
      '⚙️ Источники контекста — События календаря, древо, документы, цели, переписка, Альбом поколений, финансы, здоровье, развитие детей',
      '🚀 Roadmap — RAG-foundation и расширенная Библиотека знаний — 6–12 месяцев'
    ]
  },
  {
    id: 'chat',
