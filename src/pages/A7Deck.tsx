...
import { A7_DECK_SECTIONS } from '@/components/a7-deck/sections';
import Slide01 from '@/components/a7-deck/Slide01';
import Slide02 from '@/components/a7-deck/Slide02';
import Slide03 from '@/components/a7-deck/Slide03';
import Slide04 from '@/components/a7-deck/Slide04';
import Slide05 from '@/components/a7-deck/Slide05';
import Slide06 from '@/components/a7-deck/Slide06';
import Slide07 from '@/components/a7-deck/Slide07';
import Slide08 from '@/components/a7-deck/Slide08';
import Slide09 from '@/components/a7-deck/Slide09';
import Slide10 from '@/components/a7-deck/Slide10';
import Slide11 from '@/components/a7-deck/Slide11';
import Slide12 from '@/components/a7-deck/Slide12';
import Slide13 from '@/components/a7-deck/Slide13';
import Slide14 from '@/components/a7-deck/Slide14';
import Slide15 from '@/components/a7-deck/Slide15';
import Slide16 from '@/components/a7-deck/Slide16';
import Slide17 from '@/components/a7-deck/Slide17';
import Slide18 from '@/components/a7-deck/Slide18';

// ─── Local meeting-mode hook (независимый от strategy-v21 / bank-deck) ────────
...
function useA7DeckMeeting() {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const isMeetingMode = searchParams.get('mode') === 'meeting';

  const [activeId, setActiveId] = useState<string>(A7_DECK_SECTIONS[0].id);

  useEffect(() => {
    const handleScroll = () => {
      const probeY = SCROLL_OFFSET + 60;
      for (let i = A7_DECK_SECTIONS.length - 1; i >= 0; i--) {
        const el = document.getElementById(A7_DECK_SECTIONS[i].id);
        if (!el) continue;
        const rect = el.getBoundingClientRect();
        if (rect.top <= probeY) {
          if (A7_DECK_SECTIONS[i].id !== activeId) {
            setActiveId(A7_DECK_SECTIONS[i].id);
          }
          return;
        }
      }
  ...
  useEffect(() => {
    ...
    if (window.location.hash !== `#${activeId}`) {
      window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}#${activeId}`);
    }
  }, [activeId]);

  const activeIndex = Math.max(0, A7_DECK_SECTIONS.findIndex((s) => s.id === activeId));

  const goTo = useCallback((index: number) => {
    const clamped = Math.max(0, Math.min(A7_DECK_SECTIONS.length - 1, index));
    scrollToSection(A7_DECK_SECTIONS[clamped].id);
  }, []);
  ...
  const exitMeeting = useCallback(() => {
    navigate(`/a7-deck#${activeId}`, { replace: true });
  }, [activeId, navigate]);
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'End': {
          e.preventDefault();
          goTo(A7_DECK_SECTIONS.length - 1);
          break;
        }
        case 'Escape': {
          e.preventDefault();
  ...
  useEffect(() => {
    ...
    if (!hash) return;
    if (!A7_DECK_SECTIONS.find((s) => s.id === hash)) return;
    const t = setTimeout(() => scrollToSection(hash), 80);
    return () => clearTimeout(t);
  }, []);

  return { isMeetingMode, activeId, activeIndex, total: A7_DECK_SECTIONS.length, goNext, goPrev, exitMeeting };
}

// ─── Slide capture helper ─────────────────────────────────────────────────────

async function captureSlides(onProgress: (msg: string) => void): Promise<{ canvases: HTMLCanvasElement[]; count: number } | null> {
  const container = document.getElementById('a7-deck-content');
...
function A7AnchorNav({ activeId }: { activeId?: string }) {
  const items = A7_DECK_SECTIONS.slice(1);
  return (
    <div
      data-strategy-nav
    ...
    >
      <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto no-scrollbar pr-6" style={{ scrollPaddingRight: '1.5rem' }}>
        {items.map((it) => {
          ...
          return (
            <a
              ...
              className={`shrink-0 whitespace-nowrap text-[11px] sm:text-sm font-medium px-2.5 sm:px-3 py-1.5 rounded-full transition ${
                isActive ? 'bg-orange-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              {it.label}
            </a>
      ...
      <div aria-hidden className="pointer-events-none absolute top-0 right-0 h-full w-6 bg-gradient-to-l from-white/85 to-transparent" />
    </div>
  );
}
...
export default function A7Deck() {
  const { isMeetingMode, activeId, activeIndex, total, goNext, goPrev, exitMeeting } = useA7DeckMeeting();
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save('А7-Наша-Семья-Предложение.pdf');
  ...
  const downloadPPTX = async () => {
    ...
    try {
      ...
      pptx.subject = 'А7 × Наша семья — предложение о партнёрстве';
      pptx.title = 'А7 × Наша семья';
      ...
      await pptx.writeFile({ fileName: 'А7-Наша-Семья-Предложение.pptx' });
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 ${isMeetingMode ? 'meeting-mode' : ''}`}>
      <SEOHead title="А7 × Наша семья — Предложение о партнёрстве" path="/a7-deck" noIndex={true} />
      <PresentationHeader
      ...
      {isMeetingMode && (
        <MeetingIndicator activeIndex={activeIndex} total={total} onPrev={goPrev} onNext={goNext} onExit={exitMeeting} />
      )}
      ...
      <div id="a7-deck-content" className="max-w-5xl mx-auto px-3 sm:px-6 pt-16 pb-12 sm:pt-16">
        <Slide01 />
        <A7AnchorNav activeId={activeId} />
        <Slide02 />
        <Slide03 />
        <Slide04 />
        <Slide05 />
        ...
        <div className="my-8 flex items-center gap-4">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-slate-400 px-3">
            Четыре направления партнёрства
          </span>
          <div className="flex-1 h-px bg-slate-200" />
        </div>
        ...
        <footer className="text-center text-xs sm:text-sm text-slate-400 py-8">
          «Наша Семья» · Предложение для А7 · 2026 · частный доступ
        </footer>
      </div>
    </div>
  );
