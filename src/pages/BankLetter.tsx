...
const LETTERS: Record<LetterVariant, { subject: string; body: string }> = {
  answer: {
    subject: 'Re: стоимость подключения сервиса «Наша Семья»',
    body: `Алексей, добрый день!

Спасибо за вопрос.

Для корректности расчёта сразу уточню единицу тарификации: 1 активированное подключение = 1 Family ID = 1 семья, независимо от количества членов семьи в аккаунте.

То есть если в одном Family ID подключены, например, 2 взрослых и 2 ребёнка, для банка это всё равно одно подключение.

1. За какой период считается стоимость
Стоимость рассчитывается за одно активированное семейное подключение в месяц — то есть это ежемесячная оплата только за реально подключённые и используемые семейные аккаунты, а не разовая оплата «за подключение навсегда».

2. Зависит ли цена от числа подключений
Да, стоимость зависит от масштаба запуска:
— до 500 семей — 350 ₽ / подключение / месяц
— 1 000–4 999 семей — 149 ₽ / подключение / месяц
— 5 000+ семей — готовы обсуждать индивидуальные условия под плановый объём.

Если стартовый сценарий будет в диапазоне 500–999 семей, мы отдельно подготовим расчёт под фактический объём запуска.

3. Что входит в базовую стоимость
— доступ к основному функционалу сервиса,
— стандартные сценарии использования,
— базовая поддержка,
— хранение сервисных данных, необходимых для работы семейного аккаунта, в период действия подключения.

4. Как фиксируется верхняя граница расходов банка
Дополнительно по каждому подключению можем установить ежемесячный лимит партнёрского кошелька на 1 Family ID — например, 50 / 100 / 150 ₽ в месяц.

Это позволяет заранее зафиксировать максимальный месячный бюджет банка на одно семейное подключение.

5. Что происходит после исчерпания лимита (принципиально важно)
После исчерпания установленного лимита партнёрского кошелька новые автоматические списания на банк не производятся.

Семья продолжает пользоваться базовыми функциями, а дальнейшие платные сценарии оплачивает из собственного кошелька в сервисе.

6. Рекомендуемый рабочий формат
Для масштабного запуска мы бы рекомендовали такой стандартный вариант:
149 ₽ базовая ставка + 100 ₽ лимит партнёрского кошелька = максимум 249 ₽ / месяц за 1 Family ID (1 семью).

7. Примеры месячного бюджета банка
При варианте 249 ₽ / месяц за 1 Family ID:
— 1 000 семей — 249 000 ₽ / месяц,
— 5 000 семей — 1 245 000 ₽ / месяц.

Если подскажете ориентир по числу подключений на старте — 500 / 1 000 / 5 000 семей, — мы подготовим точный расчёт месячного бюджета под ваш сценарий.
...
2. Масштабный запуск
   1 000–4 999 семей — 149 ₽ / подключение / месяц
   5 000+ семей — индивидуальные условия под плановый объём
...
— до 500 семей — 350 ₽ / подключение / месяц
— 1 000–4 999 семей — 149 ₽ / подключение / месяц
— 5 000+ семей — индивидуально (для 500–999 — расчёт под объём)

В базовую стоимость входят основной функционал, стандартные сценарии использования, базовая поддержка и хранение сервисных данных для работы аккаунта.
...
— до 500 семей: 490 ₽ / подключение / месяц
— 1 000–4 999 семей: 199 ₽ / подключение / месяц
— 5 000+ семей: индивидуальные условия.
