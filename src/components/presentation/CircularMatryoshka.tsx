...
export function CircularMatryoshka() {
  ...
  // Подписи слоёв слева/справа. Возвращаем исходные углы — пусть линии чуть
  // задевают названия модулей, это не критично, читаемость не страдает.
  // Главное — чтобы сами подписи слоёв полностью помещались в viewBox.
  // viewBox 1200, длинные подписи ~190px → отступ 240px от центра достаточно.
  const leftX = CX - 360;
  const rightX = CX + 360;
  const labels = [
    // Слева: ЯДРО
