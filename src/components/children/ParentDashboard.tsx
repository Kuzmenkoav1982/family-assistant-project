...
export function ParentDashboard({ child }: ParentDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      <SectionHelp
        emoji="🎯"
        title="Как пользоваться родительским дашбордом"
        description="Этот дашборд помогает системно отслеживать развитие ребёнка, не упуская важные моменты. Всё в одном месте — от прививок до олимпиад."
        tips={[
          "🎯 РАЗДЕЛ РАЗВИТИЕ — создайте области (спорт, музыка, учёба), добавьте занятия (футбол по вт/чт) и следите за результатами тестов (олимпиады, соревнования)",
          "💡 Пример: добавьте область 'Спорт' → занятие 'Футбол, вт/чт 17:00, 5000₽' → результат теста 'Городские соревнования, 1 место'",
          "🧠 Кнопка 'Оценка развития' — ИИ анализирует навыки ребёнка и предлагает персональный план развития",
          "📊 Все данные в одном месте — больше не нужны блокноты и разрозненные заметки",
          "👨‍👩‍👧 Вся семья видит актуальную информацию — папа знает расписание занятий, бабушка помнит про врача"
        ]}
      />
