export const slides = [
  {
    id: 1,
    type: 'cover',
    title: '💎 Наша Семья',
    subtitle: 'Семейная операционка — ОС вашей семьи',
    content: 'Не «ещё один органайзер». Единая ОС семьи, где живут профили, отношения, договорённости, здоровье, бюджет и смыслы. Один продукт работает в 5 фазах жизни семьи: Сбор → Картина → Осмысление → Договорённости → Исполнение.',
    gradient: 'from-purple-500 via-pink-500 to-red-500'
  },
  {
    id: 2,
    type: 'problem',
    title: '🎯 Проблема',
    points: [
      'У каждого приложения только одна фаза жизни — задачи или бюджет, но никогда не семья целиком',
      'Семьи используют 5-10 разных приложений вместо одной операционки',
      'Нет единого центра: данные, отношения, договорённости, исполнение — везде по разному',
      'Информация разбросана: календари, финансы, здоровье детей, ценности, психология',
      'Сложно координировать действия всех членов семьи',
      '70% родителей испытывают стресс от хаоса в семейной организации'
    ]
  },
  {
    id: 3,
    type: 'solution',
    title: '✨ Решение — Семейная ОС',
    subtitle: '5 фаз жизни семьи в одном продукте + редкий смысловой слой',
    points: [
      '🧭 5 циклов: Сбор → Картина → Осмысление → Договорённости → Исполнение',
      '💎 Смысловой слой (наш moat) — Семейный код, Мастерская жизни, Зеркало родителя',
      '🪪 Семейный ID — единый цифровой профиль семьи для банков и маркетплейсов',
      '🌱 Портфолио развития — карта по 8 сферам для каждого члена семьи',
      '💳 Общие расходы и счета — совместный бюджет всей семьи',
      '🎁 Бонусные программы семьи — единый кошелёк лояльности',
      '🗓️ Единый семейный календарь с напоминаниями',
      '👶 Дневник здоровья детей и медкарты',
      '🍽️ Планирование меню и списки покупок',
      '✨ Навигатор мер господдержки — 40+ федеральных льгот',
      '💬 Закрытый семейный чат + интеграция с MAX (VK)',
      '🛡 Включено в Реестр российского ПО — запись №34764',
      '🤖 ИИ-помощник «Домовой» для семейных решений'
    ]
  },
  {
    id: 4,
    type: 'familyid',
    title: '🪪 Семейный ID',
    subtitle: 'Семья как единый клиент — новая парадигма',
    description: 'Единый цифровой профиль семьи открывает новое качество клиентского опыта: общие расходы, совместные счета, единый ID для банков и маркетплейсов. Сегодня такого продукта нет ни у одного игрока рынка.',
    pillars: [
      { icon: '🪪', title: 'Семейный ID', desc: 'Единая цифровая идентичность семьи — как паспорт, но для цифровых сервисов' },
      { icon: '💳', title: 'Общие расходы и счета', desc: 'Совместный бюджет, трекинг и накопления — вся семья видит общую картину' },
      { icon: '🎁', title: 'Бонусные программы семьи', desc: 'Баллы Ozon, WB, банков — в едином кошельке лояльности семьи' },
      { icon: '🔗', title: 'Интеграция с маркетплейсами', desc: 'Список покупок → Ozon/WB, чеки → семейный бюджет, ID → кросс-продажи' },
    ],
    b2b: 'Банки и маркетплейсы платят за доступ к Семейному ID — это B2B2C монетизация поверх семейного кошелька'
  },
  {
    id: 5,
    type: 'traction',
    title: '📈 Текущие показатели',
    stats: [
      { label: 'Активных семей', value: '89', icon: 'Users' },
      { label: 'Пользователей', value: '78', icon: 'UserCheck' },
      { label: 'Backend API', value: '110+', icon: 'Server' },
      { label: 'Таблиц БД', value: '245', icon: 'Database' },
      { label: 'Хабов', value: '12', icon: 'LayoutGrid' },
      { label: 'Экранов', value: '165+', icon: 'Zap' }
    ],
    note: "Production-ready · ИС защищена n'RIS №518-830-027 · в Реестре российского ПО (№34764 от 18.08.2026)"
  },
  {
    id: 6,
    type: 'tech',
    title: '🛠️ Технологии',
    subtitle: 'Современный технологический стек',
    tech: [
      { name: 'React + TypeScript', desc: '460+ компонентов, 165+ экранов' },
      { name: 'Python Backend', desc: '110+ облачных функций' },
      { name: 'PostgreSQL', desc: '245 таблиц, 290 миграций' },
      { name: 'PWA', desc: 'Работает как мобильное приложение' },
      { name: 'Яндекс.Алиса + MAX-бот', desc: 'Голос + мессенджер VK' },
      { name: 'ЮКасса + СБП', desc: 'Интеграция платежей' }
    ]
  },
  {
    id: 7,
    type: 'market',
    title: '🌍 Рынок',
    data: {
      tam: { value: '50M', desc: 'Семей в России' },
      sam: { value: '15M', desc: 'Городские семьи с детьми' },
      som: { value: '1.5M', desc: 'Целевая аудитория (3 года)' }
    },
    competitors: [
      { name: 'Todoist, Trello', issue: 'Только задачи, нет Семейного ID' },
      { name: 'Банковские приложения', issue: 'Только финансы, нет единого профиля' },
      { name: 'Ozon / Wildberries', issue: 'Только покупки, нет бонусной программы семьи' },
      { name: 'Наша семья', advantage: 'Семейный ID + единый клиентский опыт!' }
    ]
  },
  {
    id: 8,
    type: 'business',
    title: '💰 Бизнес-модель',
    subtitle: 'Семейный кошелёк — pay-per-use',
    walletInfo: 'Единый баланс семьи для AI-сервисов. Пополнение через карту или СБП (от 50 ₽). Прозрачная модель: платишь только за то, чем пользуешься.',
    services: [
      { name: 'ИИ-диета', price: '17 ₽' },
      { name: 'AI-фото блюда', price: '7 ₽' },
      { name: 'Рецепт из продуктов', price: '5 ₽' },
      { name: 'ИИ-открытка', price: '7 ₽' },
    ],
    revenue: [
      { metric: 'ARPU (ср. семья)', value: '~200 ₽/мес' },
      { metric: 'CAC (цель)', value: '₽500–800' },
      { metric: 'LTV (3 года)', value: '₽7,200' },
      { metric: 'B2B: Семейный ID API', value: 'банки и маркетплейсы' }
    ]
  },
  {
    id: 9,
    type: 'valuation',
    title: '💎 Оценка проекта',
    current: {
      title: 'Текущая стоимость',
      value: '₽42–80M',
      items: [
        'Себестоимость разработки: ₽28M',
        "ИС защищена: n'RIS №518-830-027",
        'Включено в Реестр российского ПО — запись №34764 от 18.08.2026',
        'Семейный ID — актив без аналогов на рынке',
        'Production-ready продукт с 89 семьями и 78 пользователями'
      ]
    },
    potential: {
      title: 'Потенциал при масштабировании',
      value: '₽1,2B – ₽2B',
      items: [
        '100K семей → ₽240M годовой выручки (кошелёк + партнёры)',
        'B2B: Семейный ID API для банков и маркетплейсов',
        'Интеграция с маркетплейсами — Ozon, WB, Яндекс',
        'Стратегическая продажа банку: 150–250M ₽'
      ]
    }
  },
  {
    id: 10,
    type: 'roadmap',
    title: '🗺️ План развития',
    phases: [
      {
        period: 'Сегодня · Q2 2026',
        goals: [
          '89 семей · 78 пользователей · 110+ backend-функций',
          'Навигатор мер господдержки (40+ федеральных мер) — в продакшене',
          'Реал-тайм чат семьи + интеграция с MAX-ботом',
          'Включено в Реестр российского ПО (№34764 от 18.08.2026)'
        ]
      },
      {
        period: 'Q3 2026',
        goals: [
          'Первые B2G-контракты на базе статуса в Реестре РПО',
          '500 платящих семей · кошелёк pay-per-use',
          'Пилотный регион + первый партнёрский B2G/B2B-договор'
        ]
      },
      {
        period: 'Q4 2026',
        goals: [
          '2 500 семей · точка операционной безубыточности',
          'Семейный ID API для банков (MVP)',
          'Расширение Навигатора на региональные меры'
        ]
      },
      {
        period: '2027',
        goals: [
          '6 000–10 000 семей · 5+ регионов',
          'Интеграция с Госуслугами и маркетплейсами (Ozon / WB)',
          'Бонусные программы семьи · корпоративные пакеты для работодателей'
        ]
      },
      {
        period: '2027–2028',
        goals: [
          '10 000+ семей · Series A или стратегическая продажа',
          'Выход в Казахстан / Беларусь',
          'B2B2C: банки и маркетплейсы платят за Семейный ID API'
        ]
      }
    ]
  },
  {
    id: 11,
    type: 'ask',
    title: '🚀 Инвестиционный раунд',
    ask: '₽10,000,000',
    equity: '10-15% доли',
    use: [
      { item: 'Маркетинг и привлечение пользователей', amount: '₽4,000,000' },
      { item: 'Разработка мобильных приложений iOS/Android', amount: '₽3,000,000' },
      { item: 'Команда (продакт, маркетинг, дизайнер)', amount: '₽2,000,000' },
      { item: 'Инфраструктура и масштабирование', amount: '₽1,000,000' }
    ],
    milestones: [
      '6 месяцев → 500–1 000 платящих семей, первый B2G/B2B-партнёр на базе статуса в Реестре РПО',
      '12 месяцев → 2 500 семей, точка операционной безубыточности, Семейный ID API (MVP)',
      '18 месяцев → 6 000+ семей, региональное масштабирование, готовность к Series A'
    ]
  }
];

export type Slide = typeof slides[number];