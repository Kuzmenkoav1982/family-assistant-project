...
export function AstrologyService() {
  ...
  const getDailyHoroscope = (zodiacSign: string): string => {
    // Генерируем разные прогнозы для каждого дня
    const today = new Date();
    const dayOfYear = Math.floor((today.getTime() - new Date(today.getFullYear(), 0, 0).getTime()) / 86400000);
    const seed = dayOfYear + zodiacSigns.findIndex(z => z.sign === zodiacSign);
    
    const themes = [
      { positive: 'новых начинаний', activity: 'воплощения давних идей', advice: 'проявите инициативу' },
      { positive: 'укрепления семейных связей', activity: 'домашнего уюта', advice: 'проведите время с близкими' },
      { positive: 'общения', activity: 'семейного совета', advice: 'планируйте совместные активности' },
      { positive: 'эмоциональной близости', activity: 'откровенных разговоров', advice: 'прислушайтесь к интуиции' },
      { positive: 'творчества', activity: 'организации праздника', advice: 'уделите внимание детям' },
      { positive: 'планирования', activity: 'наведения порядка', advice: 'структурируйте дела' },
      { positive: 'гармонии', activity: 'разрешения конфликтов', advice: 'найдите компромисс' },
      { positive: 'понимания', activity: 'глубоких бесед', advice: 'откройте свои чувства' }
    ];
    
    const theme = themes[seed % themes.length];
    
    return `Сегодня отличный день для ${theme.positive}! Используйте энергию для ${theme.activity}. В семейных делах ${theme.advice}.`;
  };

  const getWeeklyAdvice = (zodiacSign: string): string => {
    // Генерируем разные советы для каждой недели
    const today = new Date();
    const dayOfYear = Math.floor((today.getTime() - new Date(today.getFullYear(), 0, 0).getTime()) / 86400000);
    const weekOfYear = Math.floor(dayOfYear / 7);
    const seed = weekOfYear + zodiacSigns.findIndex(z => z.sign === zodiacSign);
    
    const weeklyThemes = [
      { focus: 'физической активности всей семьёй', action: 'Совместные прогулки или спортивные игры' },
      { focus: 'домашнему уюту и кулинарии', action: 'Семейные ужины и приготовление блюд' },
      { focus: 'общении и обучении', action: 'Семейные игры и чтение книг' },
      { focus: 'эмоциональном сближении', action: 'Создавайте семейные традиции' },
      { focus: 'творческих проектах', action: 'Организуйте семейный творческий вечер' },
      { focus: 'планировании и организации', action: 'Обновите систему семейных дел' },
      { focus: 'гармонии отношений', action: 'Ищите компромиссы и баланс' },
      { focus: 'изучении нового', action: 'Планируйте образовательные активности' }
    ];
    
    const theme = weeklyThemes[seed % weeklyThemes.length];
    
    return `На этой неделе сосредоточьтесь на ${theme.focus}. ${theme.action} укрепят связи и принесут радость.`;
  };
