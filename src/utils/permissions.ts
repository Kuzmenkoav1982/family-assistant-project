/**
 * ⚠️ УСТАРЕВШАЯ КЛИЕНТСКАЯ МАТРИЦА ПРАВ — НЕ ЯВЛЯЕТСЯ ЗАЩИТОЙ
 *
 * Источник истины по правам — версионируемая политика в backend-коде
 * (backend/_shared/auth_guard.py, ROLE_POLICY). Актуальные права текущего
 * пользователя приходят с сервера через /auth-me.
 *
 * Для нового кода используйте: import useCapabilities from '@/hooks/useCapabilities'
 *   const { can, canAsOwner } = useCapabilities();
 *   if (can('health', 'create')) { ... }
 *
 * Этот файл оставлен только на время миграции существующих экранов.
 * Он описывает ОТОБРАЖЕНИЕ (какую кнопку показать), а не доступ: любое
 * действие всё равно перепроверяется сервером и без прав вернёт 401/403.
 *
 * Не добавляйте сюда новые правила — они разойдутся с серверной политикой.
 */
