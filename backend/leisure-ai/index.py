...
def handle_recommend(body: dict, headers: dict) -> dict:
    ...
    prompt = f"""Ты - эксперт по семейному досугу. 
Порекомендуй 5-7 интересных мест для семейного отдыха в городе {city}.

Интересы семьи: {', '.join(interests) if interests else 'разнообразный досуг'}
Возрастные группы: {', '.join(age_groups) if age_groups else 'любой возраст'}
Бюджет: {budget if budget else 'не ограничен'}
Погода: {weather if weather else 'любая'}
