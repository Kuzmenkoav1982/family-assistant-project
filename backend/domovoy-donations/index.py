    ...
    try:
        ...
        if payment_method == 'wallet':
            try:
                ...
                family_id = str(fm['family_id'])
                cursor.execute(
                    "SELECT balance_rub FROM t_p5815085_family_assistant_pro.family_wallet WHERE family_id = %s FOR UPDATE",
                    (family_id,)
                )
                wallet = cursor.fetchone()
                balance = float(wallet['balance_rub']) if wallet else 0.0
                if balance < amount:
                    conn.rollback()
                    return {
                        'statusCode': 400,
                        'headers': {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'},
                        'isBase64Encoded': False,
                        'body': json.dumps({'error': f'Недостаточно средств в семейном кошельке. Баланс: {balance:.0f}₽'})
                    }
                cursor.execute(
                    "UPDATE t_p5815085_family_assistant_pro.family_wallet SET balance_rub = balance_rub - %s, updated_at = NOW() WHERE family_id = %s",
                    (amount, family_id)
                )
                cursor.execute(
                ...
                cursor.execute(
                    """INSERT INTO t_p5815085_family_assistant_pro.domovoy_levels (user_id, current_level, total_donated)
                       ...
                       SET current_level = %s, total_donated = t_p5815085_family_assistant_pro.domovoy_levels.total_donated + %s""",
                    (user_id, new_level, amount, new_level, amount)
                )
                cursor.execute(
                    """INSERT INTO t_p5815085_family_assistant_pro.domovoy_donations 
                       ...
                       VALUES (%s, %s, 'wallet', %s, 'succeeded', %s, %s)""",
                    (user_id, amount, f'wallet-{uuid.uuid4()}', current_level, new_level)
                )
                conn.commit()
                return {
                    'statusCode': 200,
                ...
                conn.rollback()
                print(f"[WALLET DONATE ERROR] {wallet_err}")
                return {
                    'statusCode': 500,
                    'headers': {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'},
                    'isBase64Encoded': False,
                    'body': json.dumps({'error': f'Ошибка списания из кошелька: {str(wallet_err)}'})
                }
