...
enqueue_portfolio_rebuild() вызывается через autocommit-соединение сразу после
source commit. Это минимизирует micro-gap между source data и queue row.

trigger_fast_path() — best-effort HTTP aggregate для низкой задержки.
force_fail=True — failpoint для smoke-тестов (aggregate пропускается).
"""
...
def enqueue_portfolio_rebuild(
    ...
    Дедупликация по member_id: несколько вызовов coalesce в одну строку очереди.
    """
    seen: set = set()
...
def trigger_fast_path(
    ...
    """Best-effort direct aggregate после commit — снижает задержку.

    Не влияет на correctness: даже если упадёт, worker подберёт из queue.
    force_fail=True — failpoint для smoke-тестов (aggregate пропускается).
    """
    if not user_id:
        return []
